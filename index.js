  ...
  try {
    ...
    app.get("/searchbus", async (req, res) => {
      try {
        ...
        if (from && to && from.toLowerCase() === to.toLowerCase()) {
          return res
            .status(400)
            .send({ message: "From and To locations can't be the same! 🚫🚌" });
        }
        ...
        if (buses.length === 0) {
          return res
            .status(404)
            .send({ message: "No buses available for the selected route." });
        }

        const reverseMatch = await busCollection
          .find({
            from: to,
            to: from,
          })
    ...
    app.patch("/buses/:id", async (req, res) => {
      ...
      if (!status) {
        return res
          .status(400)
          .send({ error: "Status field is required for update" });
      }
      ...
      try {
        const result = await busCollection.updateOne(filter, updateDoc);

        if (result.matchedCount === 0) {
          return res.status(404).send({ error: "Bus not found" });
        }

        res.send({ message: "Bus status updated successfully", result });
      } catch (error) {
        res.status(500).send({ error: "Failed to update bus status" });
      }
    });
